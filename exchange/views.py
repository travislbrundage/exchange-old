...
def unified_elastic_search_test(request, resourcetype='base'):
    mock_json = {
        'hello': 'fam'
    }
...
def unified_elastic_search2(request, resourcetype='base'):
    import requests
    import collections
    from elasticsearch import Elasticsearch
    from six import iteritems
    ...
    # only show registry, documents, layers, stories, and maps
    q = Q('match', type='layer') | Q('match', type='map') | Q(
        'match', type='story') | Q('match', type='document')
    search = search.query(q)
