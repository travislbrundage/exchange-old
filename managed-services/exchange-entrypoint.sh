...
if [[ $(grep 'http://localhost' /code/docker/exchange/docker_oauth_apps.json) ]] && [[ ${GEOSERVER_REDIRECT} ]]; then
  sed -i "s|http://localhost|${GEOSERVER_REDIRECT}|g" /code/docker/exchange/docker_oauth_apps.json
fi
